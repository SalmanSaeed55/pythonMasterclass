...
def parse_invoice_number(invoice_number: str) -> tuple[int, int]:
    ...
    """
    inv_year, inv_num = int(invoice_number.split('-')[0]), int(invoice_number.split('-')[1])
    return inv_year, inv_num
...
def next_invoice_number(invoice_number: str) -> str:
    ...
    """
    year = get_year()
    inv_year = int(invoice_number.split('-')[0])
    inv_no = int(invoice_number.split('-')[1])
    if inv_year != year:
        inv_no = "0001"
    else:
        inv_no += 1
